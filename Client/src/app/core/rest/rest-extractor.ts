...
import {dateToHuman} from "../../helpers/utils";
...
export class RestExtractor {
  ...
  handleError(err: any) {
    ...
    if (err.error instanceof Error) {
      ...
      if (err.error && err.error.errors) {
        ...
        errorMessage = `Media is too large for the server. Please contact you administrator if you want to increase the limit size.`;
  ...
  redirectTo404IfNotFound(obj: { status: number }, type: 'post' | 'other', status = [HttpStatusCode.NOT_FOUND_404]) {
