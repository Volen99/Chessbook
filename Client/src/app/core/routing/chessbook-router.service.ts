...
@Injectable()
export class ChessbookRouterService {
  ...
  getRouteSetting(snapshot?: ActivatedRouteSnapshot) {
    return (snapshot || this.route.snapshot).queryParams[ChessbookRouterService.ROUTE_SETTING_NAME];
  }
  ...
  setRouteSetting(value: number) {
    ...
    const queryParams = {[ChessbookRouterService.ROUTE_SETTING_NAME]: value};
  ...
  silentNavigate(baseRoute: string[], queryParams: { [id: string]: string }) {
    ...
    queryParams = {
      ...
      [ChessbookRouterService.ROUTE_SETTING_NAME]: routeSetting
    };
