...
import {OAuthClientLocal} from "../../shared/models";
import {HttpStatusCode} from "../../shared/core-utils/miscs";
import {objectToUrlEncoded} from "../../../root-helpers/utils";
import {UserRefreshToken} from "../../shared/models/users/user-refresh-token.model";
...
type UserLoginWithUserInformation = UserLoginWithUsername & IUser;
...
export class AuthService {
  ...
  private clientId: string = chessbookLocalStorage.getItem(AuthService.LOCAL_STORAGE_OAUTH_CLIENT_KEYS.CLIENT_ID);
  private clientSecret: string = chessbookLocalStorage.getItem(AuthService.LOCAL_STORAGE_OAUTH_CLIENT_KEYS.CLIENT_SECRET);
    ...
    this.http.get<OAuthClientLocal>(AuthService.BASE_CLIENT_URL)
      ...
      .subscribe(
        res => {
          ...
          chessbookLocalStorage.setItem(AuthService.LOCAL_STORAGE_OAUTH_CLIENT_KEYS.CLIENT_ID, this.clientId);
          chessbookLocalStorage.setItem(AuthService.LOCAL_STORAGE_OAUTH_CLIENT_KEYS.CLIENT_SECRET, this.clientSecret);
...
Ensure you have correctly configured PeerTube (config/ directory), in particular the "webserver" section.`;
          ...
          this.notifier.error(errorMessage, `Error`, 7000);
        }
      );
  }
  ...
  login(username: string, password: string, token?: string) {
    ...
    if (token) {
      Object.assign(body, {externalAuthToken: token});
    }
  ...
  private mergeUserInformation(obj: UserLoginWithUsername): Observable<UserLoginWithUserInformation> {
    ...
    return this.http.get<IUser>(AuthService.BASE_USER_INFORMATION_URL, {headers})
