...
export class CustomRequestParameters implements ICustomRequestParameters {
  ...
  get formattedCustomQueryParameters(): string {
    ...
    // new StringBuilder(`${this._customQueryParameters[0][0]}=${this._customQueryParameters[0][1]}`);
    let queryParameters = `${this._customQueryParameters[0][0]}=${this._customQueryParameters[0][1]}`;
    ...
    for (let i = 1; i < this._customQueryParameters.length; ++i) {
      queryParameters += `&${this._customQueryParameters[i][0]}=${this._customQueryParameters[i][1]}`;
    }
