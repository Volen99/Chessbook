...
namespace Chessbook.Services.Tasks
{
    ...
    public partial class TaskThread : IDisposable        // What am I doing with my life? 5/12/2021 | Lonely - Akon (Lyrics) 🎵
    {
        #region Fields
        ...
        static TaskThread()
        {
            var a = EngineContext.Current.Resolve<IStoreContext>();
            _scheduleTaskUrl = $"{a.GetCurrentStoreAsync().Result.Url.TrimEnd('/')}/{NopTaskDefaults.ScheduleTaskPath}";
        ...
        private async System.Threading.Tasks.Task RunAsync()
        {
            ...
            foreach (var taskName in _tasks.Keys)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    await logger.ErrorAsync("var message = ex.InnerException?.GetType() == typeof(TaskCanceledException) ? await localizationService.GetResourceAsync('ScheduleTasks.TimeoutError') : ex.Message;", ex);
                }
                finally
                {
                    if (client != null)
