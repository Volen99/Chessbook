namespace WorldFeed.Message.Domain.AggregateRoots.MessageAggregate
{
    ...
    using WorldFeed.Message.Domain.Common;
    ...
    public class Message : Entity<long>, IAggregateRoot
    {
        ...
        public IApp App
        {
            ...
            set
            {
                // If the app is already set, it cannot be changed. The set option here is only to allow users to set the 
                // app for messages received in response to a create request, where Twitter doesn't return the app
                if (this.app != null)
                {
