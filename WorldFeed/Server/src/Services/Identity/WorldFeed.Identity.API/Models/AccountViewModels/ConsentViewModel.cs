namespace WorldFeed.Identity.API.Models.AccountViewModels
{
    ...
    public class ConsentViewModel : ConsentInputModel
    {
        public ConsentViewModel(ConsentInputModel model, string returnUrl, AuthorizationRequest request, Client client, Resources resources)
        {
            ...
            var delete = resources.ApiResources.SelectMany(s => s.Scopes);
            ...
            ResourceScopes = resources.ApiResources.SelectMany(x => x.Scopes).Select(x => new ScopeViewModel(resources.FindApiScope(x), ScopesConsented.Contains(x) || model == null)).ToArray(); // TODO: THIS IS BUGGED!!!
        }
