namespace WorldFeed.Common.QueryGenerators.User
{
    using System;
    ...
    public class UserQueryParameterGenerator : IUserQueryParameterGenerator
    {
        ...
        public string GenerateIdOrScreenNameParameter(IUserIdentifier user, string idParameterName = "user_id", string screenNameParameterName = "screen_name")
        {
            if (user == null)
            {
                return null;
            ...
            if (user.IdStr.IsNullOrEmpty() == false)
            {
