...
function run() {
    return new Promise(async res => {
        ...
        for (const file of files) {
            ...
            const stream = null; // createReadStream(file);
...
async function getFiles() {
    ...
    const logFiles = null; // await readdir(null/*CONFIG.STORAGE.LOG_DIR*/);

    const filename = await getNewestFile(logFiles, null /*CONFIG.STORAGE.LOG_DIR*/);
    return [ join(/*CONFIG.STORAGE.LOG_DIR*/ null, filename) ];
}
