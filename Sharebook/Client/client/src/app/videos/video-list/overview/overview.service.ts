...
import { RestExtractor, ServerService } from '../../../core';
    ...
    for (const key of Object.keys(serverVideosOverview)) {
      for (const object of serverVideosOverview[key]) {
        observables.push(
          of(object.videos)
            .pipe(
              switchMap(videos => this.videosService.extractVideos({
                total: 0,
                data: videos
              })),
            ...
            .pipe(
              tap(translations => {
                for (const c of videosOverviewResult.categories) {
                  c.category.label = sharebookTranslate(c.category.label, translations);
                }
              })
            );
        }),
