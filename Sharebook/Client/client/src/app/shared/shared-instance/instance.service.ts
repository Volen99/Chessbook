...
import { MarkdownService, RestExtractor, ServerService } from '../../core';
      ...
      map(([ languagesArray, translations ]) => {
        return about.instance.languages
          .map(l => {
            ...
            return sharebookTranslate(languageObj.label, translations);
          });
      })
    );
  }
      ...
      map(([ categoriesArray, translations ]) => {
        return about.instance.categories
          .map(c => {
            ...
            return sharebookTranslate(categoryObj.label, translations);
          });
      })
    );
  }
