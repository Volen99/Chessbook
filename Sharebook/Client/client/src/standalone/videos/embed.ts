...
import { sharebookTranslate } from '../../../../shared/core-utils/i18n'
import {
  ResultList,
  ServerConfig,
  UserRefreshToken,
...
export class PeerTubeEmbed {
  ...
  displayError (text: string, translations?: Translations) {
    ...
    const translatedText = sharebookTranslate(text, translations)
    const translatedSorry = sharebookTranslate('Sorry', translations)
    ...
    errorTitle.innerHTML = sharebookTranslate('Sorry', translations)
  ...
  private async buildPlaylistManager () {
    ...
    this.player.upnext({
      timeout: 10000, // 10s
      headText: sharebookTranslate('Up Next', translations),
      cancelText: sharebookTranslate('Cancel', translations),
      suspendedText: sharebookTranslate('Autoplay is suspended', translations),
  ...
  private async buildDock (videoInfo: VideoDetails, config: ServerConfig) {
    ...
    const description = config.tracker.enabled && this.warningTitle
      ? '<span class="text">' + sharebookTranslate('Watching this video may reveal your IP address to others.') + '</span>'
      : undefined
  ...
  private async buildCaptions (serverTranslations: any, captionsResponse: Response): Promise<VideoJSCaption[]> {
    if (captionsResponse.ok) {
      ...
      return data.map(c => ({
        label: sharebookTranslate(c.language.label, serverTranslations),
  ...
  private buildPeerTubeHelpers (translations?: { [ id: string ]: string }): RegisterClientHelpers {
    ...
    return {
      ...
      translate: (value: string) => {
        return Promise.resolve(sharebookTranslate(value, translations))
      }
    }
  }
