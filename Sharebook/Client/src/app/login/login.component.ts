...
import {LOGIN_PASSWORD_VALIDATOR, LOGIN_USERNAME_VALIDATOR} from "../shared/form-validators/login-validators";
...
import {RegisteredExternalAuthConfig, ServerConfig} from "../shared/models/server/server-config.model";
...
export class LoginComponent extends FormReactive implements OnInit, AfterViewInit {
  ...
  private openedForgotPasswordModal: any; //  NgbModalRef;
  private serverConfig: ServerConfig;

  constructor(
    protected formValidatorService: FormValidatorService,
    private route: ActivatedRoute,
    private modalService: any, // NgbModal,
...
The link will expire within 1 hour.`;
  ...
  private handleError(err: any) {
    if (err.message.indexOf('credentials are invalid') !== -1) {
      this.error = $localize`Incorrect username or password.`;
    } else if (err.message.indexOf('blocked') !== -1) {
