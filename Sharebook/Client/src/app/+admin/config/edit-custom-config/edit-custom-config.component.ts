...
import { ConfigService } from '../../../+admin/config/shared/config.service';
...
export class EditCustomConfigComponent extends FormReactive implements OnInit, AfterViewChecked {
  ...
  constructor(
    ...
    this.transcodingThreadOptions = [
      {
        value: 0,
        label: $localize`Auto (via ffmpeg)`
      },
      {
        value: 1,
        label: '1'
    ...
    this.liveMaxDurationOptions = [
      ...
      {
        value: 1000 * 3600 * 5,
        label: $localize`5 hours`
      },
      {
        value: 1000 * 3600 * 10,
        label: $localize`10 hours`
      }
    ];
  }
  ...
  private loadForm() {
    forkJoin([
      ...
      ([ config, languages, categories ]) => {
        ...
        this.languageItems = languages.map(l => ({
          label: l.label,
          id: l.id
        }));
        this.categoryItems = categories.map(l => ({
          label: l.label,
          id: l.id + ''
        }));
