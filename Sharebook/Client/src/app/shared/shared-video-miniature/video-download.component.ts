...
import { VideoCaption, VideoFile, VideoPrivacy } from '../models';
...
export class VideoDownloadComponent {
  ...
  getMetadataFormat(format: any) {
    const keyToTranslateFunction = {
      'encoder': (value: string) => ({
        label: $localize`Encoder`,
        value
      }),
      'format_long_name': (value: string) => ({
        label: $localize`Format name`,
        value
      }),
      'size': (value: number) => ({
        label: $localize`Size`,
        value: this.bytesPipe.transform(value, 2)
      }),
  ...
  getMetadataStream(streams: any[], type: 'video' | 'audio') {
    ...
    let keyToTranslateFunction = {
      'codec_long_name': (value: string) => ({
        label: $localize`Codec`,
        value
      }),
    ...
    if (type === 'video') {
      keyToTranslateFunction = Object.assign(keyToTranslateFunction, {
        'width': (value: number) => ({
          label: $localize`Resolution`,
          value: `${value}x${stream.height}`
        }),
        'display_aspect_ratio': (value: string) => ({
          label: $localize`Aspect ratio`,
          value
        }),
        'avg_frame_rate': (value: string) => ({
          label: $localize`Average frame rate`,
          value
        }),
        'pix_fmt': (value: string) => ({
          label: $localize`Pixel format`,
          value
        })
      });
    } else {
      keyToTranslateFunction = Object.assign(keyToTranslateFunction, {
        'sample_rate': (value: number) => ({
          label: $localize`Sample rate`,
          value
        }),
        'channel_layout': (value: number) => ({
          label: $localize`Channel Layout`,
          value
        })
      });
    }
