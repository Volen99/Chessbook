import {NSFWQuery} from "../models/search/nsfw-query.model";
...
export class AdvancedSearch {
  ...
  constructor(options?: {
    ...
    if (isNaN(this.durationMin)) {
      this.durationMin = undefined;
    }
    if (isNaN(this.durationMax)) {
  ...
  containsValues() {
    ...
    for (const k of Object.keys(obj)) {
      if (this.silentFilters.has(k)) {
        continue;
      }

      if (this.isValidValue(obj[k])) {
        return true;
      }
    }
