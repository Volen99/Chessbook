...
import {
  ...
  VideoInfo
} from '../../../shared/models/users/user-notification.model';
import {AbuseState} from "../../models/moderation/abuse/abuse-state.model";
import {FollowState} from "../../models/actors/follow.model";
...
export class UserNotification implements UserNotificationServer {
  ...
  constructor(hash: UserNotificationServer, user: AuthUser) {
    ...
    try {
      ...
      if (this.video) {
        this.setVideoChannelAvatarUrl(this.video.channel);
      }
      ...
      if (this.actorFollow) {
        this.setAccountAvatarUrl(this.actorFollow.follower);
      }
      ...
      switch (this.type) {
        ...
        case UserNotificationType.NEW_ABUSE_FOR_MODERATORS:
          ...
          if (this.abuse.video) {
            this.videoUrl = this.buildVideoUrl(this.abuse.video);
          } else if (this.abuse.comment) {
            this.commentUrl = this.buildCommentUrl(this.abuse.comment);
          } else if (this.abuse.account) {
        ...
        case UserNotificationType.VIDEO_AUTO_BLACKLIST_FOR_MODERATORS:
          ...
          if (!this.videoBlacklist) {
            this.videoBlacklist = {id: null, video: this.video};
          }
