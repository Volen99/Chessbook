...
import {BuildFormArgument, BuildFormDefaultValues} from '../form-validators';
...
export class FormValidatorService {
  ...
  buildForm(obj: BuildFormArgument, defaultValues: BuildFormDefaultValues = {}) {
    ...
    for (const name of Object.keys(obj)) {
      ...
      if (field && field.MESSAGES) {
        validationMessages[name] = field.MESSAGES as { [name: string]: string };
      }

      const defaultValue = defaultValues[name] || '';

      if (field && field.VALIDATORS) {
        group[name] = [defaultValue, field.VALIDATORS];
      } else {
  ...
  updateForm(
    ...
    for (const name of Object.keys(obj)) {
      ...
      if (field && field.VALIDATORS) {
        form.addControl(name, new FormControl(defaultValue, field.VALIDATORS as ValidatorFn[]));
      } else {
        form.addControl(name, new FormControl(defaultValue));
      }
    }
  }
