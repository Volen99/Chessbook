...
import {VideoPlaylistElementType} from "../models/videos/playlist/video-playlist-element.model";
...
export class VideoPlaylistElementMiniatureComponent implements OnInit {
  ...
  buildRouterQuery() {
    if (!this.playlistElement || !this.playlistElement.video) {
      return {};
    }
  ...
  formatTimestamp(playlistElement: VideoPlaylistElement) {
    ...
    if (start !== null && stop === null) {
      return $localize`Starts at ` + startFormatted;
    }
    if (start === null && stop !== null) {
      return $localize`Stops at ` + stopFormatted;
    }
