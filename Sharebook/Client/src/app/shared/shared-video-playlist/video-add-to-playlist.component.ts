...
import {DisableForReuseHook} from "../../core/routing/disable-for-reuse-hook";
...
import {VideoExistInPlaylist} from "../models/videos/playlist/video-exist-in-playlist.model";
...
export class VideoAddToPlaylistComponent extends FormReactive implements OnInit, OnChanges, OnDestroy, DisableForReuseHook {
  ...
  toggleMainPlaylist(e: Event, playlist: PlaylistSummary) {
    ...
    if (this.isPresentMultipleTimes(playlist) || playlist.optionalRowDisplayed) {
      return;
    }
  ...
  onElementTimestampUpdate(playlist: PlaylistSummary, element: PlaylistElement) {
    if (!element.playlistElementId || element.enabled === false) {
      return;
    }
  ...
  private addVideoInPlaylist(playlist: PlaylistSummary, element: PlaylistElement) {
    ...
    if (element.startTimestamp) {
      body.startTimestamp = element.startTimestamp;
    }
    if (element.stopTimestamp && element.stopTimestamp !== this.video.duration) {
      body.stopTimestamp = element.stopTimestamp;
    }
    ...
    this.videoPlaylistService.addVideoInPlaylist(playlist.id, body)
      .subscribe(
        res => {
          ...
          if (element) {
            element.playlistElementId = res.videoPlaylistElement.id;
          }
        },
