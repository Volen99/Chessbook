...
import { AuthService, DisableForReuseHook, Notifier } from '../../core';
import {
  Video,
...
const logger = debug('sharebook:playlists:VideoAddToPlaylistComponent');
...
export class VideoAddToPlaylistComponent extends FormReactive implements OnInit, OnChanges, OnDestroy, DisableForReuseHook {
  ...
  private formatTimestamp(element: PlaylistElement) {
    const start = 0; // element.startTimestamp ? secondsToTime(element.startTimestamp) : '';
    const stop = 1; // element.stopTimestamp ? secondsToTime(element.stopTimestamp) : '';
