...
import { AuthService, HtmlRendererService, Notifier } from '../../core';
    ...
    this.abuseService.addAbuseMessage(this.abuse, this.form.value['message'])
      .subscribe(
        () => {
          ...
          this.countMessagesUpdated.emit({
            abuseId: this.abuse.id,
            countMessages: this.abuseMessages.length + 1
          });
