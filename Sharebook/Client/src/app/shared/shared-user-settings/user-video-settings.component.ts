...
import {SelectOptionsItem} from "../shared-forms/select/select-options.component";
...
import {ItemSelectCheckboxValue} from "../shared-forms/select/select-checkbox.component";
      ...
      this.userService.updateMyProfile(details).subscribe(
        () => {
          ...
          if (this.notifyOnUpdate) {
            this.notifier.success($localize`Video settings updated.`);
          }
        },
  ...
  private getVideoLanguages(videoLanguages: ItemSelectCheckboxValue[]) {
    ...
    if (videoLanguages.length === 1) {
      ...
      if (typeof videoLanguage === 'string') {
        if (videoLanguage === this.allLanguagesGroup) {
          return null;
        }
      } else {
        if (videoLanguage.group === this.allLanguagesGroup) {
          return null;
        }
      }
    }
