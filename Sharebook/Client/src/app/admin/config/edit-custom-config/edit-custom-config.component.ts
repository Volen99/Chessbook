...
import {
  ...
  CACHE_PREVIEWS_SIZE_VALIDATOR, INDEX_URL_VALIDATOR,
  INSTANCE_NAME_VALIDATOR,
  INSTANCE_SHORT_DESCRIPTION_VALIDATOR, SEARCH_INDEX_URL_VALIDATOR,
  SERVICES_TWITTER_USERNAME_VALIDATOR, SIGNUP_LIMIT_VALIDATOR, TRANSCODING_THREADS_VALIDATOR
} from "../../../shared/form-validators/custom-config-validators";
import {USER_VIDEO_QUOTA_DAILY_VALIDATOR, USER_VIDEO_QUOTA_VALIDATOR} from "../../../shared/form-validators/user-validators";
...
@Component({
  selector: 'app-edit-custom-config',
...
export class EditCustomConfigComponent extends FormReactive implements OnInit, AfterViewChecked {
  ...
  getTotalTranscodingThreads() {
    ...
    if (transcodingEnabled) {
      value += +transcodingThreads || 1;
    }
    if (liveTranscodingEnabled) {
      value += +liveTranscodingThreads || 1;
    }
  ...
  gotoAnchor() {
    const hashToNav = {
      customizations: 'advanced-configuration', // 'customizations'
    };
  ...
  hasConsistentOptions() {
    if (this.hasLiveAllowReplayConsistentOptions()) {
      return true;
    }
