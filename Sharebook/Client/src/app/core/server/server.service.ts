...
import { getCompleteLocale, isDefaultLocale, sharebookTranslate } from '../../shared/core-utils/i18n';
...
export class ServerService {
  ...
  private static BASE_LOCALE_URL = environment.apiUrl + '/client/locales/'; // TODO: bug
  ...
  private config: ServerConfig = {
    instance: {
      name: 'Sharebook',
      shortDescription: 'Sharebook, a federated social media platform' + 'with Angular.',
      defaultClientRoute: '',
      isNSFW: false,
        ...
        map(({
          ...
          const hashToPopulate: VideoConstant<T>[] = Object.keys(data)
            .map(dataKey => {
              ...
              return {
                id,
                label: sharebookTranslate(label, translations)
              };
            });
  ...
  private saveConfigLocally(config: ServerConfig) {
    sharebookLocalStorage.setItem(ServerService.CONFIG_LOCAL_STORAGE_KEY, JSON.stringify(config));
  }

  private loadConfigLocally() {
    const configString = sharebookLocalStorage.getItem(ServerService.CONFIG_LOCAL_STORAGE_KEY);
