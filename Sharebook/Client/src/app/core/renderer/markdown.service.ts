...
export class MarkdownService {
  ...
  private parsersConfig: MarkdownParserConfigs = {
    ...
    textWithHTMLMarkdownIt: {
      rules: TEXT_WITH_HTML_RULES,
      html: true,
      escape: true
    },
    ...
    enhancedWithHTMLMarkdownIt: {
      rules: ENHANCED_WITH_HTML_RULES,
      html: true,
      escape: true
    },
  ...
  private async createMarkdownIt(config: MarkdownConfig) {
    ...
    // const markdownIt = new MarkdownItClass('zero', {
    //   linkify: true,
    //   breaks: true,
    //   html: config.html
    // });
    ...
    return new Promise<any>(x => x); // markdownIt;
  }
