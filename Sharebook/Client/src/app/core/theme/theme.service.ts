...
import {ServerConfig, ServerConfigTheme} from "../../shared/models/server/server-config.model";
...
export class ThemeService {
  ...
  private injectThemes(themes: ServerConfigTheme[], fromLocalStorage = false) {
    ...
    for (const theme of this.themes) {
      ...
      if (fromLocalStorage === false && this.themeFromLocalStorage && this.themeFromLocalStorage.name === theme.name) {
        continue;
      }
      ...
      for (const css of theme.css) {
        ...
        if (fromLocalStorage === true) {
          this.themeDOMLinksFromLocalStorage.push(link);
        }
  ...
  private getCurrentTheme() {
    if (this.themeFromLocalStorage) {
      return this.themeFromLocalStorage.name;
    }
  ...
  private updateCurrentTheme() {
    if (this.oldThemeName) {
      this.removeThemePlugins(this.oldThemeName);
    }
