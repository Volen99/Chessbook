...
export class FileUploader {
  ...
  public cancelItem(value: FileItem): void {
    let index = this.getIndexOfItem(value);
    let item = this.queue[index];
    let prop = this.options.isHTML5 ? item._http : item._form;
  ...
  protected _xhrTransport(item: FileItem): any {
    ...
    let that = this;
    let http = item._http; // = new XMLHttpRequest();
    let sendable: any;
    ...
    // http.upload.onprogress = (event: any) => {
    //   let progress = Math.round(event.lengthComputable ? event.loaded * 100 / event.total : 0);
    //   this._onProgressItem(item, progress);
    // };
    // http.onload = () => {
    //   let headers = this._parseHeaders(http.getAllResponseHeaders());
    //   let response = this._transformResponse(http.response, headers);
    //   let gist = this._isSuccessCode(http.status) ? 'Success' : 'Error';
    //   let method = '_on' + gist + 'Item';
    //   (this as any)[method](item, response, http.status, headers);
    //   this._onCompleteItem(item, response, http.status, headers);
    // };
    // http.onerror = () => {
    //   let headers = this._parseHeaders(http.getAllResponseHeaders());
    //   let response = this._transformResponse(http.response, headers);
    //   this._onErrorItem(item, response, http.status, headers);
    //   this._onCompleteItem(item, response, http.status, headers);
    // };
    // http.onabort = () => {
    //   let headers = this._parseHeaders(http.getAllResponseHeaders());
    //   let response = this._transformResponse(http.response, headers);
    //   this._onCancelItem(item, response, http.status, headers);
    //   this._onCompleteItem(item, response, http.status, headers);
    // };
    // http.open(item.method, item.url, true);
    // http.withCredentials = item.withCredentials;
    ...
    // }
    // if (this.authToken) {
    //   http.setRequestHeader(this.authTokenHeader, this.authToken);
    // }
    // http.onreadystatechange = function() {
    //   if (http.readyState == XMLHttpRequest.DONE) {
    //     that.response.emit(http.responseText);
    //   }
    // };
    // if (this.options.formatDataFunctionIsAsync) {
    //   sendable.then(
    //     (result: any) => http.send(JSON.stringify(result))
    //   );
    // } else {
    //   debugger
    //   http.send(sendable);
