...
import {VideoShareComponent} from "../../shared/shared-share-modal/video-share.component";
import {SubscribeButtonComponent} from "../../shared/shared-user-subscription/subscribe-button.component";
import {VideoDownloadComponent} from "../../shared/shared-video-miniature/video-download.component";
import {UserVideoRateType} from "../../shared/models/videos/rate/user-video-rate.type";
import {VideoDetails} from "../../shared/main/video/video-details.model";
import {VideoCaption} from "../../shared/models/videos/caption/video-caption.model";
import {VideoPlaylist} from "../../shared/shared-video-playlist/video-playlist.model";
import {ServerConfig} from "../../shared/models/server/server-config.model";
import {VideoService} from "../../shared/main/video/video.service";
import {VideoPlaylistService} from "../../shared/shared-video-playlist/video-playlist.service";
...
import {VideoState} from "../../shared/models/videos/video-state.enum";
...
import {isXPercentInViewport, scrollToTop} from "../../helpers/utils";
import {AuthUser} from "../../core/auth/auth-user.model";
import {VideoPrivacy} from "../../shared/models/videos/video-privacy.enum";
import {VideoActionsDisplayType} from "../../shared/shared-video-miniature/video-actions-dropdown.component";
...
export class VideoWatchComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.queryParamsSub = this.route.queryParams.subscribe(queryParams => {
      ...
      if (this.player && start) {
        this.player.currentTime(parseInt(start, 10));
      }
    });
  ...
  ngOnDestroy() {
    ...
    if (this.liveVideosSub) {
      this.liveVideosSub.unsubscribe();
    }
  ...
  getVideoTags() {
    if (!this.video || Array.isArray(this.video.tags) === false) {
      return [];
    }
  ...
  private loadVideo(videoId: string) {
    ...
    if (this.video && this.video.uuid === videoId) {
      return;
    }
  ...
  private loadPlaylist(playlistId: string) {
    ...
    if (this.playlist && this.playlist.uuid === playlistId) {
      return;
    }
  ...
  private buildPlayerManagerOptions(params: {
    ...
    let mode: any; /*PlayerMode;*/

    if (urlOptions.playerMode) {
      if (urlOptions.playerMode === 'p2p-media-loader') {
    ...
    return this.peertubeSocket.getLiveVideosObservable()
      .subscribe(({type, payload}) => {
        if (type === 'state-change') {
          return this.handleLiveStateChange(payload.state);
        }
        if (type === 'views-change') {
          return this.handleLiveViewsChange(payload.views);
        }
      });
  }
  ...
  private handleLiveStateChange(newState: VideoState) {
    ...
    const videoState = this.video.state.id;
    if (videoState !== VideoState.WAITING_FOR_LIVE && videoState !== VideoState.LIVE_ENDED) {
      return;
    }
