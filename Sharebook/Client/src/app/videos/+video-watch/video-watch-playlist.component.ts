...
import {VideoPlaylistElement} from "../../shared/shared-video-playlist/video-playlist-element.model";
...
import {peertubeLocalStorage, peertubeSessionStorage} from "../../../root-helpers/sharebook-web-storage";
...
export class VideoWatchPlaylistComponent {
  ...
  onPlaylistVideosNearOfBottom(position?: number) {
    // Last page
    if (this.playlistPagination.totalItems <= (this.playlistPagination.currentPage * this.playlistPagination.itemsPerPage)) {
      return;
    }
  ...
  loadPlaylistElements(playlist: VideoPlaylist, redirectToFirst = false, position?: number) {
    this.videoPlaylist.getPlaylistVideos(playlist.uuid, this.playlistPagination)
      .subscribe(({total, data}) => {
        ...
        if (position) {
          this.updatePlaylistIndex(position);
        }
  ...
  updatePlaylistIndex(position: number) {
    if (this.playlistElements.length === 0 || !position) {
      return;
    }
