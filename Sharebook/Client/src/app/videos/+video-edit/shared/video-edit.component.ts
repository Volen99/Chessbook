...
import {FormReactiveValidationMessages} from "../../../shared/shared-forms/form-reactive";
...
} from "../../../shared/models/plugins/register-client-form-field.model";
...
import {
  ...
  VIDEO_CHANNEL_VALIDATOR, VIDEO_DESCRIPTION_VALIDATOR, VIDEO_LANGUAGE_VALIDATOR, VIDEO_LICENCE_VALIDATOR,
  VIDEO_NAME_VALIDATOR, VIDEO_ORIGINALLY_PUBLISHED_AT_VALIDATOR,
  VIDEO_PRIVACY_VALIDATOR, VIDEO_SCHEDULE_PUBLICATION_AT_VALIDATOR, VIDEO_SUPPORT_VALIDATOR, VIDEO_TAGS_ARRAY_VALIDATOR
} from "../../../shared/form-validators/video-validators";
...
export class VideoEditComponent implements OnInit, OnDestroy {
  ...
  isWaitTranscodingDisplayed() {
    if (!this.waitTranscodingEnabled) {
      return false;
    }
  ...
  private sortVideoCaptions() {
    this.videoCaptions.sort((v1, v2) => {
      if (v1.language.label < v2.language.label) {
        return -1;
      }
      if (v1.language.label === v2.language.label) {
        return 0;
      ...
      .subscribe(
        newChannelId => {
          ...
          setTimeout(() => {
            ...
            if (currentSupport && currentSupport !== oldChannel.support) {
              return;
            }
