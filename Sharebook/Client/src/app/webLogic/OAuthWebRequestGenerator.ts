...
export class OAuthWebRequestGenerator implements IOAuthWebRequestGenerator {
  generateApplicationParameters(temporaryCredentials: IReadOnlyConsumerCredentials, authRequest: IAuthenticationRequest, additionalParameters: Array<IOAuthQueryParameter>): Array<IOAuthQueryParameter> {
    return undefined;
  }
  ...
  generateAuthorizationHeaderAsync(uri: Uri, queryContent: any, httpMethod: HttpMethod, parameters: Array<IOAuthQueryParameter>): Promise<string> {
...
//             let signatureParameters = this.getSignatureParameters(oAuthQueryParameters, urlParameters);
//             let formattedUrlParameters = this.createFormattedUrlParameters(signatureParameters);
...
//
//             let dateTime = this._getUtcDateTime();
